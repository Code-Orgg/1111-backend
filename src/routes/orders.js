...
const { framingModifier, isValidFraming, DEFAULT_FRAMING } = require("../utils/framing");
...
// POST /api/orders — { items: [{ productId, qty, framing }], shipping: { fullName, email, phone, street, city, country, postalCode }, paymentMethod }
// `items[].price`/`items[].total`, if the client sends them, are always
// ignored — price is looked up server-side from the product + framing table
// so a tampered client request can never under-charge.
router.post("/", requireAuth, async (req, res) => {
  ...
  const orderItems = items.map((i) => {
    ...
    const framing = i.framing || DEFAULT_FRAMING;
    // Unit price = product's own USD price + the framing surcharge, both
    // looked up server-side — never trust a price sent from the client.
    const unitPrice = p.price + framingModifier(framing);
    return { productId: p.id, qty: i.qty, price: unitPrice, framing };
  });
  ...
  if (paymentMethod === "WALLET") {
    ...
    return res.status(201).json({ order, orderId: order.id, paid: true });
  }
  ...
  const checkout = await provider.initializeTransaction({
    ...
    // Naira gateways bill in kobo — this is the only place USD is converted.
    amountKobo: toKobo(subtotal),
    reference,
  ...
  res.status(201).json({ order, orderId: order.id, checkoutUrl: checkout.checkoutUrl });
});
