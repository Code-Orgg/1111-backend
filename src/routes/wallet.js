...
// POST /wallet/fund — { amountKobo, provider: 'PAYSTACK' | 'SQUAD' | 'MONNIFY' }
router.post("/fund", requireAuth, async (req, res) => {
  const { amountKobo, provider: providerName } = req.body;
  if (!amountKobo || amountKobo < 100000) return res.status(400).json({ error: "Minimum funding amount is ₦1,000." });
  ...
  await prisma.walletTransaction.create({
    data: { userId: req.user.id, type: "FUND", amount: amountKobo, provider: providerName, providerRef: reference, verified: false },
  });
